...
- **【9/9 定型·nextEvent.note 首句锚点铁律】新段落必须插在「下一站…出战…」首句之后，禁止 prepend 到最前面**。原理：`parseEventMeta` 用**首句里的第一个括号**当 title 锚点（`s.search(/[（(]/)` → `pm` → `after = s.slice(pm.index+len)`）。首句是「下一站…(中国澳门·东亚运动会体育馆)…」时，位次表落在 `after` 里、label 非空 → 男单 8 + 女单 4 = **12** 个 `emeta__rank-item`。若把新段插到最前，首句换人 → 首位括号变成位次表自己的 `(数字越小越靠前)` → `after` 以 `:` 开头 → `label=''` → `maybeRankList` 返回 null → 男单那 8 项退化成普通要点，**只剩 4 项**，`render.test.js`「排行榜应 >=10 名各一行」必挂。脚本写法：`first_dot = old_note.find("。"); ne["note"] = old_note[:first_dot+1] + NE + old_note[first_dot+1:]`。
- **【9/9 教训·测试里的硬编码日期会随日历失效】** `tests/logic.test.js` 曾有例把 `2026-09-11` 硬编码当「3 天后」，真实日期推进到 9/9 后它变成后天（窗口内）→ 按窗口铁律模块隐藏 → 用例误报失败。**凡「N 天后」语义的假数据，一律相对 `new Date()` 动态算**（`new Date(y, m, d + N)` 再拼 `YYYY-MM-DD`），别写死日期。
...
- **WTT 澳门冠军赛 9/8-13（澳门东亚运动会体育馆）**：冠军 1000 分；1/16 与 1/8 五局三胜、1/4 起七局四胜。央视 epg 已锁定 9/8-9/13 每日 CCTV-5+ 11:00 + CCTV-5 18:30（9/13 为 11:00 半决赛 + 18:30 决赛）。
  - **【9/9 修正·晚场窗口不是每天都到 21:30】** 逐日实核：9/8、9/9、9/11 晚场 18:30-**21:30**（《体育世界》接档）；但 **9/10 晚场 18:30 只到 20:30**（被女篮世界杯1/4决赛接走，仅 2 小时）。**别再把「晚上6点半到9点半」当全周通用模板写进 risk**，逐日查 epg。
  - **赛程分段**：9/8-9/10 白天 1/16 决赛；**9/10 晚 18:30 起 1/8 决赛 1**；9/11 全天 1/8 决赛 2；9/12 1/4 决赛；9/13 半决赛 + 决赛。9/11 起对阵依赖前一日赛果 → 按「条件场不预写」只写 schedule + pending=true。
  - **9/8 首日已全结案**：13 场比分见 `每日乒乓球赛事早报/2026-09-08.md`。冷门：林昀儒（世界第6）2-3 负 18 岁法国科顿、宇田幸矢 2-3 负黄镇廷，均一轮游；中国澳门两人（麦天恩、梁安娜）全出局。
